...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($foto!='NO_FOTO'){
		$name = "$unique$usuario$codigo$alerta$tipo$prioridad";
		$full_name = str_replace(str_split('\\/:*?"<>|%+#'),'', $name);
		$photo_name = str_replace(' ','', $full_name);
		$path_foto = "MaterialesRecibidos/$photo_name.png";
	}
	
    // Insertar meta
    $retorno = FuncionesSamsung::insertMaterialesRecibidos(
		$pharma_id,
		$codigo,
		$canal,
		$nombre_comercial,
    ...
    if ($retorno) {
		if ($foto!='NO_FOTO') {
			...
			$container = 'app/AppAlicorp/Inserts/MaterialesRecibidos';
