...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $body = json_decode(file_get_contents("php://input"), true);
		...
		if($image!='NO_FOTO'){
			...
			$name = "$unique$user$pos_id$channel";
			$full_name = str_replace(str_split('\\/:*?"<>|%+#'),'', $name);
			$photo_name = str_replace(' ','', $full_name);
			$path = "Tareas/$photo_name.png";
		} else {
			$path = "Tareas/NO_FOTO.png";
		}
    
     	
    // Insertar meta
    $retorno = FuncionesSamsung::insertTareas (
	    $pos_id,
        $user,
        $supervisor,
        $fecha,
    ...
    if ($retorno) {
        if ($image!='NO_FOTO') {
			...
			$container = 'app/AppAlicorp/Inserts/Tareas';
