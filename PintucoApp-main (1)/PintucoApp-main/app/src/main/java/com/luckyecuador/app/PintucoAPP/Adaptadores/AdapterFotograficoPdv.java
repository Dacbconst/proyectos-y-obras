...
public class AdapterFotograficoPdv extends RecyclerView.Adapter<AdapterFotograficoPdv.ViewHolder> {
    ...
    private String obtenerIdContacto(int posicion) {
        if (cursor != null) {
            if (cursor.moveToPosition(posicion)) {
                return Consultas.obtenerString(cursor, ContractInsertPdvFotografico.Columnas._ID);
            } else {
                return null;
            }
        } else {
    ...
    public AdapterFotograficoPdv(Context context) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View v = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.row10, viewGroup, false);
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int i) {
        ...
        codigo = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.PHARMA_ID));
        registro = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.CODIGO));
        coord = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.USUARIO));
        causal = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.SUPERVISOR));
        fecha = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.FECHA));
        hora = cursor.getString(cursor.getColumnIndexOrThrow(ContractInsertPdvFotografico.Columnas.HORA));
